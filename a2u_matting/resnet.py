...
class ResidualBlockWithEnFeature(nn.Module):
    """Configurable residual block
    ...
    def __init__(self,
                 ...
                 norm_act=ABN,
                 dropout=None,
                 # up_module=FeatureSelectUp
                 ):
        super(ResidualBlockWithEnFeature, self).__init__()
        ...
        #     self.up = up_module(channels[0], norm_act)
    ...
    def forward(self, x):
        ...
        if not self.is_bottleneck:
            ...
            if self.need_index:
                ...
                # x1 = InterChannelOperation.interchannel_downsampling(x0, idx_en, ik=self.downsample_kernelsize,
                #                                                      gp=self.downsample_group, ratio=2)
                x1 = F.max_pool2d(x0, kernel_size=2, stride=2)
            else:
                x1 = x0
            x = self.layer2(x1)
        else:
        ...
        if self.layer1.bn1.activation == "leaky_relu":
            if self.need_index:
                # return functional.leaky_relu(x, negative_slope=self.layer1.bn1.activation_param,
                #                              inplace=True), idx_de, x0
                return functional.leaky_relu(x, negative_slope=self.layer1.bn1.activation_param,
                                             inplace=True), x0
            else:
            ...
            if self.need_index:
                ...
                return functional.elu(x, alpha=self.layer1.bn1.activation_param, inplace=True), x0
            else:
...
class ResNet34DecoderLearning(nn.Module):
    def __init__(
        ...
        self.upsample = conf.MODEL.upsample
        ...
        for block_id in range(structure[0]):
            blocks.append(("block%d" % (block_id + 1), ResidualBlock(in_channels, channels, norm_act=norm_act, stride=1,
                                                                     dilation=try_index(dilation, 0))))
            # Update channels and p_keep
            in_channels = channels[-1]
        self.layer1 = nn.Sequential(OrderedDict(blocks))
        channels = [c * 2 for c in channels]
        # layer2
        self.layer2_1 = ResidualBlockWithEnFeature(in_channels, channels, conf, norm_act=norm_act, stride=2,
                                                   dilation=try_index(dilation, 0))
        ...
        channels = [c * 2 for c in channels]
        # layer3
        self.layer3_1 = ResidualBlockWithEnFeature(in_channels, channels, conf, norm_act=norm_act, stride=2,
                                                   dilation=try_index(dilation, 1))
        ...
        for block_id in range(structure[2] - 1):
            blocks.append(("block%d" % (block_id + 1),
                           ResidualBlock(in_channels, channels, norm_act=norm_act, stride=1,
                                         dilation=try_index(dilation, 2))))
            # Update channels and p_keep
            in_channels = channels[-1]
        ...
        ### decoder ###
        self.decoder_layer4 = Decoder(256, 128, upsample=self.upsample, conv_operator=self.decoder_conv_operator,
                                        kernel_size=self.decoder_kernel_size,
                                        batch_norm=norm_act)
        self.decoder_layer3 = Decoder(128, 64, upsample=self.upsample, conv_operator=self.decoder_conv_operator,
                                        kernel_size=self.decoder_kernel_size,
                                        batch_norm=norm_act)
        self.decoder_layer2 = Decoder(64, 64, upsample=self.upsample, conv_operator=self.decoder_conv_operator,
                                        kernel_size=self.decoder_kernel_size,
                                        batch_norm=norm_act)
        self.decoder_layer1 = Decoder(64, 32, upsample=self.upsample, conv_operator=self.decoder_conv_operator,
                                        kernel_size=self.decoder_kernel_size,
                                        batch_norm=norm_act)
    ...
    def forward(self, x):
        ...
        l0p = F.max_pool2d(l0, kernel_size=2, stride=2)
        # l0p = InterChannelOperation.interchannel_downsampling(l0, idx0_en, self.down_kernel_size,
        #                                                       self.group)  # 64x160x160

        l0_1p = F.max_pool2d(l0p, kernel_size=2, stride=2)
        # l0_1p = InterChannelOperation.interchannel_downsampling(l0p, idx1_en, self.down_kernel_size,
        #                                                         self.group)  # 64x80x80

        l1 = self.layer1(l0_1p)  # 64x80x80
        l2p, l10 = self.layer2_1(l1)
        l2 = self.layer2_2(l2p)

        l3p, l20 = self.layer3_1(l2)  # 32x80x80
        ...
        l = self.decoder_layer4(l_m, lcat_3, f_en=l20)
        lcat_2 = self.short2(l1)
        l = self.decoder_layer3(l, lcat_2, f_en=l10)
        lcat_1 = self.short1(l0p)
        l = self.decoder_layer2(l, lcat_1, f_en=l0p)
        lcat_0 = self.short0(x)
        l = self.decoder_layer1(l, lcat_0, f_en=l0)
...
def resnetmat(pretrained=False, encoder='resnet34', **kwargs):
    ...
    if encoder == 'resnet34':
        model = ResNet34DecoderLearning(**kwargs, structure=_NETS["34"]["structure"],
                                        bottleneck=_NETS["34"]["bottleneck"])
    else:
